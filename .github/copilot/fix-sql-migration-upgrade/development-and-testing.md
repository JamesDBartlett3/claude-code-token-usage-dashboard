...
## Overview
This feature fixes the issue where the SQL migration script (e.g. `001_migrate_to_model_dimension.sql`) was not automatically executed during an upgrade. The installer (`drachometer-install.py`) has been modified to automatically track and apply SQL migrations before initializing the rest of the schema.

## Changes Made
1. **Added `apply_sql_migrations()` to `drachometer-install.py`:**
   - Detects if `drachometer.db` exists.
...
2. **Updated `init_database()` in `drachometer-install.py`:**
...
3. **Fresh Database Installation:**
   - Deleted the local database entirely and ran `drachometer-install.py`.
...
Finally, we used the standard `python3 drachometer-install.py` to ensure the internal smoke test completed successfully.
